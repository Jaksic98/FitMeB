...
**2026-07-29 (odluka korisnika): ceo WhatsApp/Infobip sloj uklonjen** (`WhatsAppSender`, `InfobipWhatsAppSender`, `MetaCloudApiWhatsAppSender`, `NoopWhatsAppSender`, `WhatsAppSenderConfig`). Zamenjen novim `EmailService` interfejsom (trenutno `LoggingEmailService`, log-only stub dok se ne uvede **Brevo**).
...
- [ ] Prava email integracija preko **Brevo** (zamena za `LoggingEmailService`) — ostaje za implementaciju.
