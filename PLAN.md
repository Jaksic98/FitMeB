...
## Modul 9 — Notifikacije (§8 SPEC.md) — WhatsApp/Infobip uklonjen, prelazak na email/Brevo

**2026-07-29 (odluka korisnika): ceo WhatsApp/Infobip sloj uklonjen** (`WhatsAppSender`, `InfobipWhatsAppSender`, `MetaCloudApiWhatsAppSender`, `NoopWhatsAppSender`, `WhatsAppSenderConfig`). Zamenjen novim `EmailService` interfejsom.

- [x] **OTP verifikacija broja telefona** (`PhoneVerificationService`) — vraćena, ali kod se sada šalje na `user.email` umesto WhatsApp-a na `phoneNumber` (odluka korisnika: identifikacija i dalje po `phoneNumber` u `/api/auth/phone/send-otp`/`verify-otp`, ali dostava koda preko emaila — ovim se efektivno verifikuje i mejl i telefon). `otp_hash`/`otp_expires_at`/`phone_verified` kolone privremeno uklonjene pa vraćene (`V12` → `V14` drop → `V15` re-add, jer se već primenjena Flyway migracija ne sme menjati). `INACTIVE → ACTIVE` ide preko uspešne OTP verifikacije; email-link aktivacija (`ActivationTokenService`) ostaje neaktivna u `AuthService.register` (endpoint `/api/auth/activate` i dalje postoji kao neiskorišćen fallback).
- [x] Podsetnici (`AppointmentReminderService`/`AppointmentReminder`/`ReminderScheduler`) — scheduling logika nepromenjena, slanje ide kroz `EmailService` na `user.getEmail()` (`email.templates.reminder`).
- [x] **Prava email integracija preko Brevo (2026-07-29)** — `LoggingEmailService` zamenjen sa `BrevoEmailService`/`BrevoEmailClient`. `BrevoEmailClient` šalje `POST https://api.brevo.com/v3/smtp/email` (autentikacija preko `api-key` header-a, konfiguracija `brevo.api-key`/`brevo.sender.email`/`brevo.sender.name` ← `BREVO_API_KEY`/`BREVO_SENDER_EMAIL`/`BREVO_SENDER_NAME` env promenljive). `BrevoEmailService.sendTemplate` učitava HTML fajlove iz `src/main/resources/email-templates/{templateName}.html` (`fitme_otp.html`, `fitme_reminder.html`) i zamenjuje `{{0}}`, `{{1}}`, ... placeholder-e pozicionim indeksom — odluka korisnika: folder sa HTML fajlovima, ne inline HTML u kodu i ne Brevo dashboard template ID-jevi. Greške iz Brevo API-ja se umotavaju u `EmailSendException` (unchecked, bez `ErrorCode`-a jer se uvek hvata interno) — pozivaoci (`PhoneVerificationService`, `AppointmentReminderService`, `NewsletterService`) je hvataju i loguju bez propagacije ka klijentu.
- [x] **Newsletter (2026-07-29)** — `EmailService.sendRaw(toEmail, subject, htmlContent)` dodat za ad-hoc sadržaj (bez lokalnog template fajla). `NewsletterService.sendNewsletter` šalje svim `ACTIVE` korisnicima sa `emailNotifications=true` (`UserRepository.findByStatusAndEmailNotifications`), broji uspele/neuspele pojedinačno (jedan neuspeh ne prekida batch) i vraća `NewsletterSendResultDTO`. Endpoint `POST /api/newsletter/send` (`NewsletterController`, ADMIN-only) prima `SendNewsletterRequestDTO` (subject + htmlContent, admin ih unosi ručno svaki put — nema Brevo Campaigns/kontakt liste integracije, odluka korisnika: samo transakcioni API).
